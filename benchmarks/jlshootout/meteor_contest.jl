...
function generateBitmasks()
    masksAtCell = Array{Array{UInt64,1}}(width*height, height)
...
function solveCell(cell_, board::UInt64, n, solutions, masks, masksAtCell)
    ...
    if board & (UInt64(1) << cell_) != 0
        # Cell full
        solveCell(cell_ - 1, UInt64(board), n, solutions, masks, masksAtCell)
        return
    end
    ...
    for color in 0:9
        if masks[color + 1] == 0
            for mask in masksAtCell[cell_ + 1, color + 1]
                if legal(mask, board)
                    masks[color + 1] = mask
                    solveCell(cell_ - 1, UInt64(board | mask), n, solutions, masks, masksAtCell)
                    masks[color + 1] = 0
                end
            end
        end
...
function solve(n, solutions, masks)
    masksAtCell = generateBitmasks()
    solveCell(width*height-1, UInt64(0), n, solutions, masks, masksAtCell)
end
