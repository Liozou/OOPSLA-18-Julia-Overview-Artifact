...
The Julia REPL (read-eval-print loop) can be launched by simply executing the command `julia` in a console.

Installing a package can be done by executing the command
```julia
Pkg.add("PackageName")
```
...
The following section explains how to reproduce the results detailed in section 6 of our paper.

##### Data collection

Once a number of packages have been tested and their `.log`, `.dyn` and `.static` have been written to disk in the `logs/` folder, the precise metrics can be automatically collected by executing `analytics/collect_data.jl`. The last line of this file will launch the data collection itself, whose result is stored in the `logs/data/` folder.

Two directories are also created, `logs/static_soft/` and `logs/static_strict/` which contain `.static` files analogous to those in `logs/static/`. They correspond to the functions that passed either the _soft_ or the _strict_ elimination, as defined in section 6.2.1. of our paper.
...
```bash
$ cd ~/julia; git checkout *; make
```
The recompilation should take less than fifteen minutes.

The different optimization cuts observed in Fig. 22 can be reproduced with the following settings:
